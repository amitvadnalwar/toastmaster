...
import {
  ...
  Maximize, ArrowRight, ClipboardList, QrCode, Users, CheckCircle, UserX, BarChart3, UserPlus,
} from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-28 max-w-lg mx-auto w-full">
        {editing ? (
          ...
          <>
              ...
              <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-2xl p-4 mb-5">
                <div>
                  ...
                  <p className="text-base font-bold text-gray-900">
                    {myAssignment.role === 'speaker' ? 'Speaker' : myAssignment.role.replace(/_/g, ' ')}
                  </p>
            ...
            {/* Admin action buttons — compact, side by side */}
            ...
            {isAdmin && meeting.status !== 'draft' && (
              <NavButton
                ...
                label="Guests"
                onClick={() => navigate(`/meetings/${id}/guests`)}
              />
            )}
            {isAdmin && meeting.status !== 'draft' && (
              <NavButton
        ...
        <div className="w-[70px] flex justify-end gap-1">
          {editing ? (
            <button onClick={onSave} disabled={acting || saveDisabled} className="p-2">
              {savingText ? <Spinner size="sm" /> : <Check size={20} className="text-green-500" />}
            </button>
          ) : isAdmin ? (
            canEdit ? (
              <>
