...
export default function MemberMeetingDetailPage() {
  ...
  if (fetching) {
    return (
      <div className="flex flex-col min-h-full bg-gray-50">
        <Header
          title={editing ? 'Edit Meeting' : 'Meeting Details'}
          isAdmin={isAdmin}
          editing={editing}
          onBack={() => navigate(isAdmin ? '/meetings' : '/meetings')}
  ...
  const canApply = !isAdmin && isPublished && !isPast;
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-28 max-w-lg mx-auto w-full">
        {editing ? (
          ...
          <>
            ...
            {canManageMeeting && meeting.status === 'draft' && (
            ...
            {canManageMeeting && meeting.status === 'published' && (
              <button
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-4">
              ...
              {isAdmin && canManageMeeting && speakers.length < meeting.max_speakers && (
                <div>
              ...
              {isAdmin && !canManageMeeting && speakers.length === 0 && (
...
function Header({
  title, isAdmin, editing, onBack, onScan, canScan, canEdit, onStartEdit, onDelete, onSave, onCancelEdit, acting, saveDisabled,
}: {
  ...
  acting?: boolean; saveDisabled?: boolean;
}) {
  return (
    <div className="bg-white border-b border-gray-100 px-4 py-3 sticky top-0 z-20">
      <div className="max-w-lg mx-auto flex items-center justify-between">
        {editing ? (
          <button onClick={onCancelEdit} className="text-brand font-semibold text-base w-[70px] text-left">Cancel</button>
        ) : (
        ...
        <div className="w-[70px] flex justify-end gap-1">
          {editing ? (
            <button onClick={onSave} disabled={acting || saveDisabled} className="p-2">
